...
pub struct DefaultJointConstraintSet<N: RealField, Handle: BodyHandle> {
    ...
    NonlinearConstraintGenerator<N, Handle> + Downcast + Send + Sync
{
    ...
    /// `body1` and `body2` should be fetched using `JointConstraint::anchors`.
    ///
    /// Typically, a constraint is disable if it is between two sleeping bodies, or, between bodies without any degrees of freedom.
    fn is_active(&self, body1: &dyn Body<N>, body2: &dyn Body<N>) -> bool {
